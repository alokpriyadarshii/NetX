*** Settings ***
Documentation  Simple functional tests for NetX
...
*** Variables ***
${BINARY_PATH}  ${CURDIR}/../../NetX
${PORT}  55353


*** Settings ***
Test Teardown  Stop NetX
...
Start NetX
  ...
  Sleep  0.5
  Is Process Running  ${netx}
  Common Test Setup

Start NetX With Valgrind
  [Arguments]  @{args}
  ...
  Sleep  6  # wait for valgrind to fire up NetX
  Is Process Running  ${netx}
  Common Test Setup

Stop NetX
  Send Signal To Process  SIGINT  ${netx}
  ${result} =  Wait For Process  ${netx}  timeout=15 secs
  IF  $result is None
...
Handle Unbound Server Does Not Support HTTP/1.1
  Start NetX  -x  -r  https://doh.mullvad.net/dns-query  # resolver uses Unbound
...
Source Address Binding
  ...
  Start NetX  -S  ${eth0_ip}  -b  1.1.1.1,8.8.8.8  -r  https://dns.google/dns-query
  ...
  # Verify NetX works (HTTPS connection uses source binding)
  Run Dig
...
Source Address Binding IPv6 With IPv4 Only Mode
  ...
  # Start NetX with IPv6 address in IPv4-only mode (-4 flag)
  Start NetX  -4  -S  ::1  -b  1.1.1.1,8.8.8.8  -r  https://dns.google/dns-query
  Sleep  1s
...
Source Address Binding Invalid Address
  ...
  # Start NetX with invalid IP address
  Start NetX  -S  not-an-ip-address  -b  1.1.1.1,8.8.8.8  -r  https://dns.google/dns-query
  Sleep  1s
